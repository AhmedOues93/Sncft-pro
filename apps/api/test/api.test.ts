...
import { previewFaresCsv, previewSchedulesCsv } from '../src/services/imports.js';
...
function seedPublishedData() {
  memoryStore.drafts.clear();
  memoryStore.published = {};

  const schedule = previewSchedulesCsv(scheduleCsv);
  const scheduleDraft = memoryStore.createDraft('schedule', schedule.stops, schedule.summary, schedule.issues);
  memoryStore.publishDraft(scheduleDraft.id);

  const fares = previewFaresCsv(faresCsv);
  const fareDraft = memoryStore.createDraft('fare', fares.fares, fares.summary, fares.issues);
  memoryStore.publishDraft(fareDraft.id);
}
...
test('health + stations + direct/transfer journey search with pagination and fare logic', async () => {
  ...
  await withServer(async (base) => {
    ...
    const directPage1Json = await directPage1.json() as {
      ...
      items: Array<{ departureTime: string; fare: { amount: number } }>;
    };
    ...
    assert.equal(directPage1Json.total, 5);
