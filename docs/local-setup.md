...
## 3.1) Batch import all CSV files (A/D/E schedules + fares)
...
## 7) Production persistence safety check

In production mode, memory store fallback is blocked. The API will fail fast unless Supabase is configured:

```bash
NODE_ENV=production STORAGE_DRIVER=memory npm run dev:api
# expected: startup error that STORAGE_DRIVER must be supabase
```
